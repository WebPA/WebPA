...
class FormRenderer {
	...
	public $assessment_feedback_title = 'Feedback / Justification';
	...
	*/
	...
	function draw_description() {
		if ($this->_form->type =='split100') {
			...
			?>
			<p>For each criterion you must rate your teammates using the scale provided.  High scores indicate better performance in the criteria.</p>
			<?php
		}
	}// /->draw_description()
	...
	*/
		...
		if ($this->_form->type=='split100') {
			...
			for ($q=0; $q<$question_count; $q++) {
				...
				<div class="question">
					...
					<?php
						// header row
						echo('<tr><td style="background-color: transparent">&nbsp;</td><th align="center">score</th><td style="background-color: transparent">&nbsp;</td></tr>');
						...
						echo("<tr><th style=\"text-align: right;\">Total&nbsp;</th><th><div style=\"background-color: $bgcolor;\" id=\"total_{$q}\">$initial_total</div></th>");
						echo('<td style="background-color: transparent"><input type="button" name="recalc_'. $q .'" value="recalculate" onclick="calcTotal('. $q .','. $participant_count .')"></td>');
						echo("</tr>\n");
					?>
					</table>
				</div>
	...
	?>
				...
				<p><b><?php echo($this->assessment_feedback_title); ?></b></p>
					<p>This section of the assessment is for you to provide general feedback and justification of the scores you have awarded in the section above.</p>
