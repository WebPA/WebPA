...
class UI
{
    ...
    public function __construct($_user = null)
    {
        ...
        $this->set_menu('Support', [
            'help' => $helper_link, // this is a link set in each page / area to link to the appropriate help
            'contact' => APP__WWW . '/contact/',
        ]);
        ...
        if ($this->user) {
            ...
            if ((($_source_id == '') || $this->user->is_admin()) && (count($modules) > 1)) {
    ...
    public function head()
    {
        ...
        <style type="text/css">
            <?php if (!isset($_SESSION['_no_header'])) : ?>
            #app_bar {
    ...
    {
        $this->menu[$section_name] = $section_array;
    }

    public function get_menu($section_name)
    {
        if (isset($this->menu[$section_name])) {
            return $this->menu[$section_name];
        }
    ...
    public function menu()
    {
        ...
        if ($this->menu) {
            ...
            foreach ($this->menu as $menu_section => $menu_links) {
    ...
    {
        $this->page_bar_buttons[$side][$text] = array('img' => "../images/buttons/$img", 'link' => $link);
    }
    ...
    {
        if (is_array($this->page_bar_buttons)) {
            ?>
            <div id="page_bar">
                <table cellpadding="0" cellspacing="0">
                    <tr>
                        <?php
                        if (array_key_exists('left', $this->page_bar_buttons)) {
                            foreach ($this->page_bar_buttons['left'] as $text => $button) {
                        ...
                        if (array_key_exists('right', $this->page_bar_buttons)) {
                            foreach ($this->page_bar_buttons['right'] as $text => $button) {
            ...
            <div style="margin-top: 50px">
                ...
                <span style="font-size: small;">Version: <?php
                    ...
                    if (count($INSTALLED_MODS) > 0) {
                        echo ' [' . implode(",", $INSTALLED_MODS) . ']';
                    }
                    ?>
      </span>
                <?php
                if ($_source_id && $_user !== null && $_user->is_admin()) {
                    echo "<br />\n";
                    echo '      <span style="font-size: small;">Source:&nbsp;';
                    echo $_source_id ?: '&lt;' . APP__NAME . '&gt;';
                    echo "</span>\n";
                }
                ?>
            </div>
