...
<?php
	...
	function process_form() {
		...
		if ($send_email_to=='groups') {
			...
			if ( ($num_students===0) ) { $errors[] = 'The group(s) you have selected contain no students. There must be at least one recepient for this email.'; }
		}
		
		$this->wizard->set_field('email_subject', fetch_POST('email_subject'));
		if (is_empty($this->wizard->get_field('email_subject'))) { $errors[] = 'You must enter a subject for this email.'; }

		$this->wizard->set_field('email_text', fetch_POST('email_text'));
		if (is_empty($this->wizard->get_field('email_text'))) { $errors[] = 'You must enter the text of this email.'; }
