...
if ($type == 'view') {
	...
	foreach($member_names as $i => $member) {
		...
		echo('<tr>');
		echo("<td style=\"text-align: left\"> {$member['surname']}, {$member['forename']} ({$member['institutional_reference']})</td>");
		echo("<td>$score</td>");
...
if ($type == 'download-csv') {
	...
	echo('"name","Web-PA score","Intermediate Grade","Non-Submission Penalty","Final grade"'."\n");
	...
	foreach($member_names as $i => $member) {		
		...
		echo("\"{$member['surname']}, {$member['forename']} ({$member['institutional_reference']})\",\"$score\",\"$intermediate_grade\",\"$penalty_str\",\"$grade\"\n");
	}
}
...
if ($type == 'download-xml') {
	...
	foreach($member_names as $i => $member) {		
		...
		echo("\t\t\t<surname>{$member['surname']}</surname>\n");
		echo("\t\t</name>\n");
		echo("\t\t<institutional_student_number>{$member['institutional_reference']}</institutional_student_number>\n");
...
if ($type == 'download-moodle-xml'){
	...
	foreach($member_names as $i => $member) {	
		...
		echo("<student>{$member['institutional_reference']}</student>");
...
if ($type == 'download-rtf'){
	...
	foreach($member_names as $i => $member) {
		...
		echo "\nStudent Number:\t{$member['institutional_reference']}";
		echo "\nWebpa Score:\t{$score}";
