...
<?php
	...
	function form() {
		...
		$errors = null;
		if (!$form) {
			$errors[] = gettext('Unable to load the form that this question belongs to.');
		} else {
		...
		if (is_array($errors)) {
			...
			echo('<p><strong>'.gettext('Unable to create your new assessment criterion.').'</strong></p>');
			echo('<p>'.gettext('To correct the problem, click <em>back</em> and amend the details entered.').'</p>');
		} else {// Else.. create the form!
			?>
			<p><strong><?php echo gettext('Your changes to this criterion have been saved.');?></strong></p>
			<p style="margin-top: 20px;"><?php echo sprintf(gettext('You can now return to <a href="/tutors/forms/edit/edit_form.php?f=%s">editing your form</a>.'), $form->id);?></p>
			<script language="JavaScript" type="text/javascript">
			<!--
				function body_onload() {
