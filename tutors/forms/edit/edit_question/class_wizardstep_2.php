...
?>
    <p><?php echo sprintf(gettext('Your new assessment criterion allows scores from %d to %d You can use the boxes below to provide a description what those scores should mean.'), $range_start, $range_end);?></p>
    ...
    <div class="form_section">
      ...
      <?php
        for($i=$range_start; $i<=$range_end; $i++) {
          echo('<tr>');
          echo("<th><label for=\"scorelabel{$i}\">".gettext('Score')." $i</label></th>");
          ...
          if ($i==$range_start) {
            echo('<td style="font-size: 0.9em; font-style: italic;">'.gettext('Lowest').'</td>');
          } else {
            if ($i==$range_end) {
              echo('<td style="font-size: 0.9em; font-style: italic;">'.gettext('Highest').'</td>');
            } else {
              echo('<td>&nbsp;</td>');
            }
          }
