...
require_once("../library/classes/class_user.php");
...
switch($_POST['action'])
{
	case "init":
		...
		$hash = md5(rand());
		$uid = $DB->fetch_value("SELECT user_id FROM user WHERE username = '$_POST[username]'");
		if(!$uid)
		{
		...
		$sql = <<<SQL
		CREATE TABLE IF NOT EXISTS user_reset_request (
...
SQL;
		...
		$sql = "INSERT INTO user_reset_request SET hash = '$hash', user_id = $uid";
		//echo $sql;
...
TXT;
		//echo $email;
		$uemail = $DB->fetch_value("SELECT email FROM user WHERE user_id = $uid");
		mail($uemail,APP__NAME. " Password Reset",$email,"From: no-reply@cse.unsw.edu.au");
	...
	case "reset":
		...
		$hash = $_POST['hash'];
		$uid = $_POST['uid'];
		$rslt = $DB->fetch_value("SELECT COUNT(*) FROM user_reset_request WHERE hash = '$hash' AND user_id = $uid");
		if($rslt)
		{
			if($_POST['newpass']==$_POST['confirmpass'])
			{
				$user = new User();
				$user_row = $DB->fetch_row("SELECT * FROM user WHERE user_id = $uid");
				$user->load_from_row($user_row);
				$user->set_dao_object($DB);
				$user->update_password(md5($_POST['newpass']));
				$user->save_user();
				
				$DB->execute("DELETE FROM user_reset_request WHERE user_id = $uid");
...
$UI->menu_selected = '';
$UI->breadcrumbs = array	('Password Reset' 			=> null ,
													);
$UI->help_link = null;
