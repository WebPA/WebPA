...
*/
	...
	*/
	function create() {
		...
		while (true) {
			$new_id = uuid_create();
			if ($this->_DAO->fetch_value("SELECT COUNT(group_id) FROM user_group WHERE group_id='$new_id' ")==0) { break; }
		}
		$this->id = $new_id;
	}// ->create()
	...
	*/
	function delete() {
		if ($this->_collection->is_locked()) {
			...
			$this->_DAO->execute("DELETE FROM user_group WHERE group_id='{$this->id}' ");
			$this->_DAO->execute("DELETE FROM user_group_member WHERE group_id='{$this->id}' ");
			return true;
		}
	}// /->delete()
	...
	*/
	function load($id) {
		$row = $this->_DAO->fetch_row("SELECT * FROM user_group WHERE group_id='$id' LIMIT 1");
	...
	*/
	function save() {
		if ( (!$this->id) || ($this->_collection->is_locked()) ) {
			...
			// Save the Group
			$this->_DAO->do_insert("REPLACE INTO user_group ({fields}) VALUES ({values}) ",$_fields);

			// Save the Group's members by deleting them all, then re-inserting
			$this->_DAO->execute(	"DELETE FROM user_group_member
			...
			if (count($this->_members)>0) {
				...
				$this->_DAO->do_insert_multi("REPLACE INTO user_group_member ({fields}) VALUES {values} ", $fields);
			}
...
*/
	...
	*/
	function refresh_members() {
		$this->_members = $this->_DAO->fetch_assoc("SELECT user_id, user_role
													FROM user_group_member
													WHERE group_id='{$this->id}' AND collection_id='{$this->_collection->id}'
